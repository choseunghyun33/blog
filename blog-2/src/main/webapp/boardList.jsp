...
<div class="container">
	...
	<div class="row">
		...
		<div class="col-sm-10">
			...
			%>
			<% 
				if(session.getAttribute("loginLevel") != null // null이 아니고
					&& (Integer)(session.getAttribute("loginLevel")) > 0){ // 레벨(session은 object 형변화)이 0보다클경우 
			%>
					<div>
						<a href="./insertBoard.jsp">글입력</a>
					</div>
			<% 		
				}
			%>
			...
			<div>
				...
				<%			
						...
						if(currentPage > 1){
				%>
						<li class="page-item"><a class="page-link" href="./boardList.jsp?currentPage=<%=currentPage-1%>&word=<%=word%>">이전</a></li>
				<%				
						}
					} else if(locationNo != null && word != null){ // 지역번호 검색 둘 다 있을때
						if(currentPage > 1){
				%>
						<li class="page-item"><a class="page-link" href="./boardList.jsp?currentPage=<%=currentPage-1%>&locationNo=<%=locationNo%>&word=<%=word%>">이전</a></li>
				<%				
						}
					}
				%>
				...
				<%			
						...
						if(currentPage < lastPage){
				%>
						<li class="page-item"><a class="page-link" href="./boardList.jsp?currentPage=<%=currentPage+1%>&word=<%=word%>">다음</a></li>
				<%				
						}
					} else if(locationNo != null && word != null){ // 지역번호 검색 둘 다 있을때
						if(currentPage < lastPage){
				%>
						<li class="page-item"><a class="page-link" href="./boardList.jsp?currentPage=<%=currentPage+1%>&locationNo=<%=locationNo%>&word=<%=word%>">다음</a></li>
				<%				
						}
					}
				%>
